...
\scriptsize
\begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{2.3cm} >{\RaggedRight\arraybackslash}p{1.6cm} >{\RaggedRight\arraybackslash}p{1.8cm} >{\RaggedRight\arraybackslash}p{2.3cm} >{\raggedleft\arraybackslash}p{1.2cm} Y}
\toprule
...
Crossref & 08/07/2026 & 2010--2026 & 5 consultas em \texttt{query.\allowbreak bibliographic} & 1.000 & Fonte complementar; 200 resultados por consulta, ordenados por relevância. \\
\midrule
...
\node (final) [below=of nucleo2, draw=black, very thick] {Auditoria qualitativa estruturada \\ \textbf{104 registros no núcleo original}; 33 não retidos};
\draw[arrow] (bruto) -- (unico);
\draw[arrow] (unico) -- (nucleo1);
\draw[arrow] (nucleo1) -- (nucleo2);
\draw[arrow] (nucleo2) -- (final);
\end{tikzpicture}
\caption{Fluxo de seleção da busca principal até o núcleo temático original}
\label{fig:funil}
\end{figure}
