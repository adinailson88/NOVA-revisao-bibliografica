...
A rotina não usa similaridade aproximada de títulos e não estabelece vínculo entre trabalho em evento e artigo posterior quando os identificadores diferem. Por isso, versões relacionadas podem permanecer separadas. Em sentido oposto, os 173 grupos fundidos apenas por título normalizado constituem o conjunto de maior risco de fusão indevida. Não houve revisão manual integral dos 1.808 grupos, de modo que a deduplicação é reprodutível e conservadora quanto aos conflitos detectados, mas não elimina toda possibilidade de duplicata remanescente ou agrupamento incorreto.
...
A pré-triagem dos 9.542 registros foi executada por regras determinísticas aplicadas a título e resumo. Uma amostra estratificada de 100 registros, sorteada com semente 42, foi lida individualmente por um único avaliador; 34 classificações foram ajustadas. Os outros 9.442 registros mantiveram a classe automática e não foram verificados individualmente nessa auditoria. Um corte inicial reteve 3.786 registros: 3.472 classificados como relevantes e 314 casos de dúvida com apenas o bloco de objeto predial presente. Em seguida, os 4.276 registros classificados como dúvida receberam decisão registro a registro em tabela auditável: 206 foram considerados relevantes e 4.070 foram descartados. A aplicação dessas decisões reduziu o núcleo em 108 registros, consolidando 3.678 registros no núcleo analítico revisado.
...
A auditoria qualitativa estruturada dos 137 registros manteve 104 no núcleo principal. Entre os 33 não retidos no núcleo final, 21 foram classificados como secundários, três apenas para mapeamento e nove foram excluídos da síntese. Um desses nove havia sido inicialmente marcado para verificação pontual em texto completo e foi posteriormente descartado sem leitura integral. Não houve segundo avaliador independente. A Tabela~\ref{tab:funilselecao} e a Figura~\ref{fig:funil} apresentam o fluxo.
